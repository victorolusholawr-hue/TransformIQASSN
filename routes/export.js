...
const { buildBrd, buildFrd, buildRiskRegister, buildExecutiveSummary, buildFutureState } = require('../services/exportBuilder');
...
async function doExport(res, req, docType, bufferFn, filename) {
  try {
    ...
    await pool.request()
      ...
      .query('INSERT INTO dbo.Documents (id, project_id, doc_type, file_url, generated_by) VALUES (@id, @pid, @type, @url, @uid)');
...
router.post('/projects/:projectId/export/executive-summary', analystRequired, projectAccessRequired, (req, res) =>
  doExport(res, req, 'executive_summary', () => buildExecutiveSummary(req.params.projectId, req.project.name),
